...
async function request<T>(
  ...
  try {
    ...
    // caller-supplied "application/json" here would silently strip that
...
export const stocksApi = {
  ...
  listReports: (ticker: string) =>
    request<DseReportsListResponse>(
      `/stocks/${encodeURIComponent(ticker)}/reports`,
      { timeoutMs: 15_000 },
    ),

  uploadReport: (ticker: string, fiscalYear: string, file: File) => {
    const form = new FormData();
    form.append("fiscal_year", fiscalYear);
    form.append("file", file);
    return request<DseReportUploadResponse>(
      `/stocks/${encodeURIComponent(ticker)}/reports/upload`,
